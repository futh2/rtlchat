...
function bindUI() {
  ...
  $('btnWorkspacePanel')?.addEventListener('click', async () => {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    try {
...
async function requestSitePermission(origin) {
  if (!origin) return false;
  if (await hasSitePermission(origin)) return true;
...
async function toggleCurrentSite() {
  ...
  if (!origin) {
    if (sub) sub.textContent = 'لا يمكن تفعيل الإضافة على صفحات المتصفح الداخلية';
    return;
  }

  if (title) title.textContent = 'بانتظار السماح من كروم';
  if (sub) sub.textContent = 'وافق على إذن الموقع لتفعيل RTL Free هنا';

  const allowed = await requestSitePermission(origin);
  if (!allowed) {
    renderState();
    if (sub) sub.textContent = 'لم يتم منح السماح لهذا الموقع';
    return;
  }
...
function renderAll() {
  ...
  $('fontWeightVal').textContent = settings.fontWeight;
  if ($('forceRTL')) $('forceRTL').checked = settings.forceRTL;
  if ($('hideTashkeel')) $('hideTashkeel').checked = settings.hideTashkeel;
  if ($('fixInputs')) $('fixInputs').checked = settings.fixInputs;
...
function renderState() {
  ...
  if (active) {
    ...
    sub.textContent = currentHost ? 'سيطلب كروم السماح لهذا الموقع فقط' : 'افتح موقعًا لتفعيل الإضافة';
  }
  renderSiteLabel();
}
...
function loadPreviewFont(fontFamily) {
  if (!PREVIEW_FONTS.includes(fontFamily) || loadedPreviewFonts.has(fontFamily)) return;
